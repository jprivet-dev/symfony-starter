...
.PHONY: drop
drop: _doctrine confirm ## Drop the database [y/N] | [a=<arguments>] | a="--env=test"
ifneq ($(IS_SQLITE),)
...
.PHONY: create
create: _doctrine ## Create the database | [a=<arguments>] | a="--env=test"
ifneq ($(IS_SQLITE),)
...
.PHONY: diff
diff: _doctrine ## Generate a migration by comparing your current database to your mapping information (format the generated SQL) | [a=<param>] | a="--profile"
	$(CONSOLE) doctrine:migrations:diff --formatted -v $(a)

.PHONY: execute
execute: _doctrine ## Execute one or more migration versions up or down manually | a=<arguments> | a="DoctrineMigrations\Version20240205143239"
...
.PHONY: migrate
migrate: _doctrine ## Execute a migration to the latest available version (in a transaction) | [a=<param>] | a="current+3"
	$(CONSOLE) doctrine:migrations:migrate --no-interaction --all-or-nothing $(a)

.PHONY: migration
migration: _doctrine ## Create (via MakerBundle) a new migration based on database changes (format the generated SQL) | [a=<param>] | a="--profile"
	$(CONSOLE) make:migration --formatted -v $(a)

##

.PHONY: fixtures
fixtures: _doctrine ## Load fixtures (CAUTION! The load command purges the database) | [a=<param>] | a="--append"
...
.PHONY: validate
validate: _doctrine ## Validate the mapping files | [a=<arguments>] | a="--env=test"
