...
CONTRIB_MONOREPO_DEFAULT  = ../symfony
CONTRIB_MONOREPO_PATH     = $(or $(CONTRIB_MONOREPO_LOCAL_PATH),$(CONTRIB_MONOREPO_DEFAULT))
CONTRIB_MONOREPO_COMPOSER = $(CONTRIB_MONOREPO_PATH)/composer.json

_monorepo: # INTERNAL - Check if local Symfony monorepo is correctly mounted
ifeq ($(wildcard $(CONTRIB_MONOREPO_PATH)),)
	@printf " $(R)❌ Error: $(CONTRIB_MONOREPO_PATH) does not exist on your machine.$(S)\n"
	@printf " $(Y)›$(S) Check $(Y)CONTRIB_MONOREPO_LOCAL_PATH$(S) in your $(G).env.local$(S) file if you don't use $(G)$(CONTRIB_MONOREPO_DEFAULT)$(S) by default\n"
	@exit 1
endif
