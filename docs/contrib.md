...
You can add the `SYMFONY_MONOREPO` variable to your `.env.local` file.
...
touch .env.local
echo "SYMFONY_MONOREPO=../symfony" >> .env.local
```
...
This is the magic command. It replaces the `vendor/symfony/*` packages in your application with symbolic links pointing to your local `SYMFONY_MONOREPO`.
